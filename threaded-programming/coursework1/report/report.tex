...
From these results, the scheduling option with the best performance for each loop on 4 threads was determined. Using these two options, the best for loop 1 and the best for loop 2, the code was then run again on 1, 2, 4, 6, 8, 12 and 16 threads. This allowed the measurement of the speedup, as defined by $T_{p}/T_{1}$, so that the scalability of the best performing schedule on 4 threads could be analysed.
...
\centering
  ...
  \caption{\small Speed Up for the dynamic 8 schedule}
  \label{dynamic8}
\end{figure}
...
\begin{figure}
  ...
  \caption{\small Speed Up for the dynamic 16 schedule}
  \label{fig:sub2}
\label{dynamic16}
\end{figure}

After the best schedules for loop 1 and loop 2 had been found they were then run again on  
