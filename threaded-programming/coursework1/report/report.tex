...
The first results gathered were execution times for each loop for every available loop schedule. The results for these are shown in Figures \ref{loop1scheds} and \ref{loop2scheds}. For the loop 1 results the 

\begin{figure}[h]
\centering
\includegraphics[width=1\textwidth]{"graphs/loop1_blocksize"}
\caption{Graph showing execution time for loop 1 against block size for the guided, dynamic and static schedules. The error bars show 1 standard deviation. Note that these error bars are much larger than in loop 2. }
\label{loop1scheds}
\end{figure}
...
\caption{Graph showing execution time for loop 2 against block size for the guided, dynamic and static schedules.}
\label{loop2scheds}
\end{figure}
